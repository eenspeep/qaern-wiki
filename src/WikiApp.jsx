...
import { RichEditor, InfoboxEditor } from './RichEditor'
...
// RichEditor and InfoboxEditor are imported from ./RichEditor
...
function MobileToolbar({ onArticles, onBulletin, onInitiative, onTracker, onMap, onDowntime, onForum, onChangelog, onRoster }) {
...
export default function WikiApp() {
  ...
  useEffect(() => {
    ...
    else if (hash === 'roster')     setShowRoster(true)
    ...
    <div style={{display:'flex',flexDirection:'column',height:'100vh',overflow:'hidden',fontFamily:"'Source Serif 4',Georgia,serif",background:theme.bg,color:theme.text,transition:'background 0.3s,color 0.3s'}}>
          ...
          <div ref={moreRef} style={{position:'relative',flexShrink:0}}>
            ...
            {showMore&&(
              <div style={{position:'absolute',top:'calc(100% + 4px)',right:0,background:theme.bgAlt,border:`1px solid ${theme.border}`,borderRadius:4,boxShadow:'0 4px 16px rgba(0,0,0,0.12)',zIndex:100,minWidth:160,overflow:'hidden'}}>
                {[
                  {label:'⚔ Roster',    action:()=>{setShowRoster(true);    history.replaceState(null,'','#roster')}},
      ...
      {isMobile && (
        <MobileToolbar
          ...
          onRoster={()=>{ setShowRoster(true);    history.replaceState(null,'','#roster') }}
        />
      )}

      {showRoster     && <Roster          user={user} onClose={()=>{ setShowRoster(false);     history.replaceState(null,'',window.location.pathname) }}/>}
